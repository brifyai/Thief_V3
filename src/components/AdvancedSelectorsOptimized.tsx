...
import { ChevronDown, ChevronRight, TestTube, Save, Trash2, HelpCircle, AlertTriangle } from 'lucide-react';
...
export const AdvancedSelectorsOptimized: React.FC<AdvancedSelectorsProps> = ({ 
  onSelectorsChange, 
  initialSelectors 
}) => {
  ...
  const memoizedSelectors = useMemo(() => selectors, [selectors]);

  const handleSelectorChange = useCallback((field: keyof CustomSelectors, value: string) => {
    const newSelectors = { ...selectors, [field]: value };
    setSelectors(newSelectors);
    onSelectorsChange?.(newSelectors);
  }, [selectors, onSelectorsChange]);

  const clearSelectors = useCallback(() => {
    const emptySelectors: CustomSelectors = {};
    setSelectors(emptySelectors);
    onSelectorsChange?.(emptySelectors);
    setTestResults(null);
  }, [onSelectorsChange]);

  const validateSelectors = useCallback((): string[] => {
    const errors: string[] = [];
    
    if (!testUrl) {
      errors.push('Debes ingresar una URL para probar');
    }
    
    try {
      new URL(testUrl);
    } catch {
      errors.push('La URL ingresada no es válida');
    }
    
    if (!selectors.titleSelector && !selectors.contentSelector) {
      errors.push('Debes configurar al menos un selector de título o contenido');
    }
  ...
  const testSelectors = async () => {
    ...
    try {
      const result = await scrapingService.testSelectors({
        url: testUrl,
      ...
      if (result.success) {
        toast.success('✅ Selectores probados exitosamente');
      } else {
        toast.error(`❌ Error: ${result.error || 'Error desconocido'}`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error al probar los selectores';
  ...
  const showSelectorHelp = useCallback(() => {
    toast(`
...
💡 Tip: Usa las herramientas de desarrollador del navegador (F12) para inspeccionar elementos.
  ...
  const hasRequiredSelectors = useMemo(() => {
    return selectors.titleSelector || selectors.contentSelector;
  }, [selectors]);
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={isLoading || !hasRequiredSelectors || !testUrl}
                variant="outline"
              >
            ...
            {!hasRequiredSelectors && (
              <p className="text-xs text-amber-600 flex items-center gap-1">
                ...
                Debes configurar al menos un selector de título o contenido
              </p>
            )}
          </div>
